...
3. Launch 실행
    * `$ roslaunch msg_send m_send.launch` 실행 결과
    ![런치 실행 확인](img/screenshot-05.png)
---
## **1:N, N:1, N:N 통신**
1. 1:N: 한 개의 publisher node에서 보낸 토픽을 N개의 subscriber node가 받음
    ...
    *launch에서 지정된 이름으로 node가 생성되고 연결된다.*
        ...
        ```
    ![1:N 실행결과](img/screenshot-06.png)
2. N:1: N 개의 publisher node에서 보낸 토픽을 1개의 subscriber node가 받음
    ...
    2. `m_send_n1.launch`
        ...
        ```
    ![N:1 실행결과](img/screenshot-07.png)
3. N:N
    1. 위의 예제와 똑같은 파일을 사용하고, launch 파일만 변경해 준다.
    2. `m_send_nn.launch`
        ```html
        ...
        ```
        ![N:N 실행결과](img/screenshot-08.png)
## **나만의 메세지 만들기**
1. 나만의 메세지를 만들기 위해서는 패키지 폴더 안에 msg 폴더와 msg 파일을 만들어야 한다.
    ![msg 폴더 위치](img/screenshot-09.png)
    * msg 폴더 및 메세지 파일 생성
        ![msg 폴더 생성](img/screenshot-10.png)
    * msg 파일 생성
        ![msg 파일 생성 및 내용](img/screenshot-11.png)
